export const DATA_EXTRACTION_QUEUE_NAME="data-extraction"
export const DATA_EXTRACTION_EVENT = "data-extraction-event"
export const DATA_EXTRACTION_GRAIP_CALLBACK_EVENT = "data-extraction-graip-callback-event"

export const TRADE_TRUST_QUEUE_NAME="trade-trust"
export const TRADE_TRUST_PREPARE_ISSUE_EVENT="trade-trust-prepare-issue-event"
export const TRADE_TRUST_ISSUE_EVENT="trade-trust-issue-event"
export const TRADE_TRUST_MINT_EVENT="trade-trust-mint-event"
