...
import { computeDocumentHashFromDataUrl, extractDocumentTrackingId, fileToDataUrl } from '../utils/document-utils';
...
export class VerifyTradeDocumentService {
  ...
  async verifyTradeDocument(file: Express.Multer.File):Promise<TradeDocumentVerificationResultsDto> {

    // Extract Tracking Id from the file
    const {documentTrackingId, documentHash} = await this.extractVerificationDetailsFromFile(file);
  ...
  private async extractVerificationDetailsFromFile(file: Express.Multer.File) {
    ...
    try {
      ...
      this.logger.error({message: "Failed to extract verification details from file", error: error.message});
      return {
  ...
  private async performVerification(documentTrackingId: string, fileDocumentHash: string):Promise<TradeDocumentVerificationResultsDto> {
    // Retrieve the stored Trade Document Details
    const verificationDetails = await this.tradeDocumentsRepository.getDocumentByTrackingId(documentTrackingId, ['verifiableDataUrlHash', 'issueDetails', 'accountId'], [])
    if (!verificationDetails) {
      this.logger.debug({message: "failed to obtain verification details", documentTrackingId})
      return {
        validationSuccess: false,
    ...
    const accountDetails = await this.accountsRepository.findAccountById(verificationDetails.accountId);
  ...
  async getTradeDocumentByTrackingId(trackingId: string) {
    const document = await this.tradeDocumentsRepository.getDocumentByTrackingId(trackingId, [], ['wrappedContent', 'verifiableDataUrlHash', 'tradeDocumentFile', 'merkleRoot']);
    if (!document) {throw new NotFoundException(`Trade document with tracking id ${trackingId} not found`)}
    return document;
  }

  async getTradeDocumentFileByTrackingId(trackingId: string) {
    const document = await this.tradeDocumentsRepository.getDocumentFileDetailsByTrackingId(trackingId);
    if (!document) {throw new NotFoundException(`Trade document with tracking id ${trackingId} not found`)}
    return document;
  }
}
